...
To execute in Windows and get only the response execute as such:
```bash
$response = Invoke-WebRequest -Uri 'http://localhost:5000/classify' -Method 'POST' -Headers @{'Content-Type'='application/json'} -Body '{ "title": "my product title containing aquarelle" }'
